  ...
  const validateField = (name: string, value: string) => {
    ...
    switch (name) {
      case "username":
        ...
        else if (value.trim().length < 3) error = "اسم المستخدم يجب أن يكون 3 أحرف على الأقل";
        break;
      case "password":
        if (!value.trim()) error = "كلمة المرور مطلوبة";
        else if (value.trim().length < 4) error = "كلمة المرور يجب أن تكون 4 أحرف على الأقل";
        break;
    }
    ...
    <form onSubmit={handleSubmit} className="space-y-6">
      ...
      <div>
        ...
        <input
          ...
          style={{
            borderColor: errors.password && touched.password ? "#ef4444" : "var(--color-border)",
            backgroundColor: "var(--color-background)",
            color: "var(--color-text)",
          }}
          required
